const state={items:[],selected:new Set(),neededFrom:""};
...
function dateCompatibility(x){
 if(!state.neededFrom)return {rank:0,label:""};
 if(x.availability?.status==="unavailable")return {rank:3,label:"Non disponibile"};
 const a=x.availability?.availableFrom;
 if(!a)return {rank:2,label:"Data da verificare"};
 return a<=state.neededFrom?{rank:0,label:"Compatibile con la tua data"}:{rank:1,label:`Disponibile solo dal ${availabilityText(x)}`};
}
function visibleItems(){return [...state.items].sort((a,b)=>dateCompatibility(a).rank-dateCompatibility(b).rank);}
function setNeededFrom(v){state.neededFrom=v;render();}
function render(){
 const root=document.getElementById("v2-root"); if(!root)return;
 root.innerHTML=`<section class="panel"><h2 class="section-title">Quando ti serve?</h2><label class="note" for="neededFrom">Mi serve dal</label><input id="neededFrom" type="date" value="${state.neededFrom}" onchange="setNeededFrom(this.value)"><p class="note">Non nascondiamo automaticamente le alternative: quelle compatibili vengono prima; quelle disponibili più tardi o con data incerta restano visibili e chiaramente segnalate.</p></section><section class="panel"><h2 class="section-title">Schede V2 pilota</h2><p class="note">Dataset interno: le schede non autorizzate restano escluse dalla pubblicazione reale.</p></section>`+visibleItems().map(x=>{const c=dateCompatibility(x);return `
 <section class="card">
  ...
  <div><span class="badge warn">Costo ${costText(x)}</span><span class="badge warn">${statusText(x)}</span>${c.label?`<span class="badge ${c.rank===0?"":"warn"}">${c.label}</span>`:""}</div>
...
function comparison(){const xs=state.items.filter(x=>state.selected.has(x.id));if(xs.length<2)return"";const row=(label,fn)=>`<div class="row"><span>${label}</span><strong>${xs.map(fn).join(" · ")}</strong></div>`;return `<section class="panel"><h2 class="section-title">Confronto V2</h2>${row("Costo mensile",costText)}${row("Disponibilità",availabilityText)}${state.neededFrom?row("Rispetto alla tua data",x=>dateCompatibility(x).label):""}${row("Ultima conferma",x=>x.availability?.confirmedAt?fmtDate(x.availability.confirmedAt):"da riconfermare")}${row("Sede",x=>x.campusReference?.campusName||"—")}${row("Spese",x=>x.validation?.expensesDeclared?"dichiarate":"da verificare")}${row("Autorizzazione",x=>x.publication?.authorized?"sì":"no")}</section>`;}
