...
function features(p, room) {
  const out = [];
  if (p.wifi) out.push('Wi-Fi'); if (p.washingMachine) out.push('Lavatrice'); if (p.dishwasher) out.push('Lavastoviglie');
  if (p.elevator) out.push('Ascensore'); if (room?.balcony || p.balcony) out.push('Balcone'); if (p.furnished) out.push('Arredato');
  if (p.dryer) out.push('Asciugatrice');
  return out;
}
function campus(p) {
  const z = Array.isArray(p.nearbyZones) ? p.nearbyZones[0] : null;
  return z
    ? { campusId: 'centro', campusName: it(z.title) || 'Polo universitario', distanceKm: null, minutesBike: n(z.bikeMinutes), travelEstimate: false }
...
function normalize(x) {
  ...
  return {
    ...
    details: {
      ...
      contract: min ? 'Permanenza minima ' + min + ' mesi' : 'Da verificare nell’annuncio Phosphoro',
      deposit: dep != null ? '€ ' + dep : 'Da verificare',
      bathroom: room.privateBathroom ? 'Privato' : 'Condiviso',
      floor: p.floor != null ? String(p.floor) : 'Da verificare'
    },
    availability: {
    ...
    accommodationType: x.type && String(x.type).includes('stanza') ? 'room' : 'apartment'
  };
}
